          ...
          if exitCode == 0:
            ...
            if exitCode == 0:
              let dot = readFile(folder / "code.svg").strip.multiReplace(@[
                ...
                ("transform=\"scale(1 1) rotate(0) ", "transform=\"scale(0.55 0.75) rotate(0) "),
              ...
              if exitCode == 0:
                ...
                if exitCode == 0:
                  if likely(targets != "e"): (output, exitCode) = execCmdEx("upx -q -f --brute " & folder / "code" & (if isWin: ".exe" else: ""))
                  when not defined(release): echo exitCode, "\tupx"
                  if exitCode == 0:
                    let fsize = if likely(targets in ["c", "cpp", "objc"]): getFileSize(folder / "code" & (if isWin: ".exe" else: "")).int div 1024 else: 0
